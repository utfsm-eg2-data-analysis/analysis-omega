...
#include "GetSimpleTuple_basic.h"
...
int main(int argc, char **argv) {
  ...
  // define TIdentificatorBasic
  TIdentificatorBasic *t = new TIdentificatorBasic(input);
...
void printUsage() {
  std::cout << "GetSimpleTuple_basic program. Usage is:" << std::endl;
  std::cout << std::endl;
  std::cout << "./GetSimpleTuple_basic -h" << std::endl;
  std::cout << "    prints usage and exit program" << std::endl;
  std::cout << std::endl;
  std::cout << "./GetSimpleTuple_basic -t[target]" << std::endl;
  std::cout << "    selects target, which can be: C, Fe, Pb" << std::endl;
  std::cout << std::endl;
  std::cout << "./GetSimpleTuple_basic -r[run number]" << std::endl;
...
int parseCommandLine(int argc, char* argv[]) {
  int c;
  if (argc == 1) {
    std::cerr << "Empty command line. Execute ./bin/GetSimpleTuple_basic -h to print usage." << std::endl;
    exit(1);
  }
  while ((c = getopt(argc, argv, "ht:r:")) != -1)
    ...
    default:
      std::cerr << "Unrecognized argument. Execute ./bin/GetSimpleTuple_basic -h to print usage." << std::endl;
      exit(0);
      break;
    }
}
...
void printOptions() {
  std::cout << "Executing GetSimpleTuple_basic program. The chosen parameters are: " << std::endl;
