...
TString outDir = proDir + "/out/SystPi0Mass";
...
inline int parseCommandLine(int argc, char* argv[]) {
  ...
  while ((c = getopt(argc, argv, "hp:f:mk:i:w:")) != -1)
    switch (c) {
    ...
    case 'i': pi0Nsigma = atof(optarg); break;
    case 'w': wNsigma = atof(optarg); break;
    default:
...
void printUsage() {
  ...
  std::cout << "  i[3-5]    :  with [3-5]*sigma cut around pi0 peak" << std::endl;
  std::cout << "  w[3-5]    :  with [3-5]*sigma cut around omega peak" << std::endl;  
  std::cout << std::endl;
}
...
void assignOptions() {
  ...
  if (plotFlag) {
    ...
    wNSigmaSufix = Form("-%fw", wNsigma);
    pi0NSigmaSufix = Form("-%fpi0", pi0Nsigma);
    outPrefix = "nbs-MR" + kinvarMRSufix;
    outputTextName = outDir + "/" + outPrefix + wNSigmaSufix + pi0NSigmaSufix + ".dat";
    canvasWidth = 1200;
    canvasHeight = 1000;
  }
  // output name
  outputPlotName = outDir + "/" + outPrefix + wNSigmaSufix + pi0NSigmaSufix + ".png";
}
...
void fitOmegaPeak() {
  ...
  Double_t wFitRangeDown = wMeanIGV - 5*wSigmaIGV;
  Double_t wFitRangeUp = wMeanIGV + 5*wSigmaIGV;
  ...
  for (Int_t index2 = 0; index2 < kinvarNbins; index2++) {
    ...
    RooRealVar wx("wx", "IMD(#omega) (GeV)", wFitRangeDown, wFitRangeUp); // updated
    ...
    RooPlot *wFrame = wx.frame(Title("IMD(#pi^{+} #pi^{-} #pi^{0}) for D Data"), Bins(wNbins));
    ...
    // save canvas
    c4f->Print(outDir + "/wfit-D" + fKinvarAuxSufix + ".png"); // output file
  }
}
