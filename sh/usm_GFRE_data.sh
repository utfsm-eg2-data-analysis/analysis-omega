...
#############################################################
# ./usm_GFRE_data.sh <target>                               #
#     <target> = (C, Fe, Pb)                                #
#                                                           #
# EG: ./usm_GFRE_data.sh C                                  #
#     ./usm_GFRE_data.sh Fe                                 #
#     ./usm_GFRE_data.sh Pb                                 #
#############################################################
...
FNCDIR=${HOME}/analysis-omega/FilterNCombine
...
cp ${FNCDIR}/bin/FilterNCombine_data ${OUDIR2}
cp ${REDDIR}/bin/Reductor            ${OUDIR3}
cp ${FNCDIR}/bin/FilterNCombine_data ${OUDIR4}
...
for ((COUNTER=1; COUNTER <= ${NFILES}; COUNTER++)); do # ${NFILES} or 1
    ...
    echo "./FilterNCombine_data -t${TARNAME} -r${RN}"                 >> ${jobfile} # execute program
