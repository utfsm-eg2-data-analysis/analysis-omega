...
OUDIR=${PRODIR}/out/FilterNCombine/${setName}/${tarName}
mkdir -p ${OUDIR}
mkdir -p ${OUDIR}/tmp
...
while [ ${COUNTER} -lt ${lines} ]; do
    ...
    jobfile="${OUDIR}/tmp/FNC_${setName}${tarName}_${rn}.sh"
    jobname="FNC_${setName}${tarName}_${rn}"
    ...
    echo "./bin/FilterNCombine -${inputOption} -t${tarName} -r${rn}" >> ${jobfile}
