...
void Get_ElectronNumbers(TString targetOption) {
  ...
  TCut CutRegion_GST = "Q2 > 1. && Q2 < 4.1 && Nu > 2.2 && Nu < 4.25";  // GST format, different from omega
  ...
  Int_t eNumber = electronsChain->Draw("Nu>>data(200, 2.2, 4.25)", gCutDIS && CutVertex && CutRegion_GST, "goff");
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t i = 0; i < Nbins; i++) {
      ...
      eNumber = electronsChain->Draw("Nu>>data2(200, 2.2, 4.25)", gCutDIS && CutVertex && CutBin && CutRegion_GST, "goff");
      // print results
      if (i == 0) {
