...
void Get_ElectronNumbers() {
  // Count the number of electrons from data, according to eta binning
  ...
  TString OutputFileName = gProDir + "/macros/include/EtaElectronNumbers.hxx";
  std::ofstream OutputTextFile{OutputFileName, std::ios::out};
  OutputTextFile << "// This library has been created automatically by \"eta_reco/Get_ElectronNumbers.cxx\"" << std::endl;
  ...
  for (Int_t t = 0; t < Ntargets; t++) {
    ...
    eNumber_int[t] = electronsChain[t]->Draw("Nu>>data(200, 2.2, 4.25)", gCutDIS && CutVertex[t] && CutRegion_GST, "goff");
    // write them onto output library
    if (t == 0) {
      OutputTextFile << "const Double_t kNElec_Eta[" << Ntargets << "] = {" << eNumber_int[t] << ", ";
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      for (Int_t i = 0; i < Nbins; i++) {
        ...
        // get electron numbers
        eNumber_diff[t][k][i] =
            electronsChain[t]->Draw("Nu>>data2(200, 2.2, 4.25)", gCutDIS && CutVertex[t] && CutRegion_GST && CutBin, "goff");
        // write them onto output library
        if (i == 0 && t == 0) {
          OutputTextFile << "const Double_t kNElec" << kinvarOption[k] << "_Eta[" << Ntargets << "][" << Nbins << "] = {{"
                         << eNumber_diff[t][k][i] << ", ";
