...
void Get_ElectronNumbers() {
  // Count the number of electrons from MC, according to eta binning
  ...
  TCut CutRegion_GST = "mc_Q2 > 1. && mc_Q2 < 4.1 && mc_Nu > 2.2 && mc_Nu < 4.25";  // GST format
  ...
  TString OutputFileName = gProDir + "/macros/include/EtaMCElectronNumbers.hxx";
  std::ofstream OutputTextFile{OutputFileName, std::ios::out};
  OutputTextFile << "// This library has been created automatically by \"eta_mc/Get_ElectronNumbers.cxx\"" << std::endl;
  ...
  for (Int_t t = 0; t < Ntargets; t++) {
    ...
    eNumber_int[t] = electronsChain[t]->Draw("mc_Nu>>data(200, 2.2, 4.25)", gCutDIS_MC && CutRegion_GST, "goff");
    // write them onto output library
    if (t == 0) {
      OutputTextFile << "const Double_t kNElec_EtaMC[" << Ntargets << "] = {" << eNumber_int[t] << ", ";
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      for (Int_t i = 0; i < Nbins; i++) {
        ...
        // get electron numbers
        eNumber_diff[t][k][i] = electronsChain[t]->Draw("mc_Nu>>data2(200, 2.2, 4.25)", gCutDIS_MC && CutBin, "goff");
        // write them onto output library
        if (i == 0 && t == 0) {
          OutputTextFile << "const Double_t kNElec" << kinvarOption[k] << "_EtaMC[" << Ntargets << "][" << Nbins << "] = {{"
                         << eNumber_diff[t][k][i] << ", ";
