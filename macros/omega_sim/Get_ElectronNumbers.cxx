...
void Get_ElectronNumbers() {
  // Count the number of electrons from sim, according to omega binning
  ...
  TString OutputFileName = gProDir + "/macros/include/OmegaSimElectronNumbers.hxx";
  std::ofstream OutputTextFile{OutputFileName, std::ios::out};
  OutputTextFile << "// This library has been created automatically by \"omega_sim/Get_ElectronNumbers.cxx\"" << std::endl;
  ...
  for (Int_t t = 0; t < Ntargets; t++) {
    ...
    eNumber_int[t] = electronsChain[t]->Draw("Nu>>sim(200, 2.2, 4.2)", gCutDIS && CutRegion_GST, "goff");
    // write them onto output library
    if (t == 0) {
      OutputTextFile << "const Double_t kNElec_OmegaSim[" << Ntargets << "] = {" << eNumber_int[t] << ", ";
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      for (Int_t i = 0; i < Nbins; i++) {
        ...
        eNumber_diff[t][k][i] =
            electronsChain[t]->Draw("Nu>>sim2(200, 2.2, 4.2)", gCutDIS && CutBin, "goff");
        // write them onto output library
        if (i == 0 && t == 0) {
          OutputTextFile << "const Double_t kNElec" << kinvarOption[k] << "_OmegaSim[" << Ntargets << "][" << Nbins << "] = {{"
                         << eNumber_diff[t][k][i] << ", ";
