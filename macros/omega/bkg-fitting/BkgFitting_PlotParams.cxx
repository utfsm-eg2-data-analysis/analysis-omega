...
void BkgFitting_PlotParams() {
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      rootInputFile[k][t] = new TFile(gProDir + "/macros/bkg-fitting/bkg-fitting_binned_" + targetString[t] + "_" + kinvarOption[k] + ".root");
      for (Int_t i = 0; i < Nbins; i++) {  // i for bins

	RooFitResult* FitResult = rootInputFile[k][t]->Get<RooFitResult>(Form("fit-result_%d_%d", i, k));
	...
	std::cout << "fitOmega = " << fitOmega[k][t][i] << " +- " << fitOmegaError[k][t][i] << std::endl;
