...
void PPlot_ElectronNumbers_simrec() {
  ...
  for (Int_t t = 0; t < Ntargets; t++) {
    ...
    for (Int_t i = 0; i < Nbins; i++) {
      // fill Q2 arrays
      electronNumber[0][t][i] = kNElecQ2_Sim[t][i];
      electronNumberErr[0][t][i] = TMath::Sqrt(electronNumber[0][t][i]);
      // fill Nu arrays
      electronNumber[1][t][i] = kNElecNu_Sim[t][i];
  ...
  const Int_t Nx = 2;
  const Int_t Ny = 1;
  TCanvas *c = new TCanvas("pplot-electron-numbers_simrec", "pplot-electron-numbers_simrec", 1200, 600);
