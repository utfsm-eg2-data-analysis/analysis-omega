...
void RadCorr_MakeMR() {
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      // data
      rootInputFile[k][t] =
          new TFile("evnt-mixing_" + targetString[t] + "_" + kinvarOption[k] + "_rc.root");
      ...
      for (Int_t i = 0; i < Nbins; i++) {
        ...
        */
        ...
        if (k == 0) {
          electronHist[k][t]->SetBinContent(i + 1, kNElecQ2_rc[t][i]);
          electronHist[k][t]->SetBinError(i + 1, TMath::Sqrt(kNElecQ2_rc[t][i]));
        } else if (k == 1) {
          electronHist[k][t]->SetBinContent(i + 1, kNElecNu_rc[t][i]);
          electronHist[k][t]->SetBinError(i + 1, TMath::Sqrt(kNElecNu_rc[t][i]));
        } else if (k == 2 || k == 3) {
          electronHist[k][t]->SetBinContent(i + 1, kNElec_rc[t]);
          electronHist[k][t]->SetBinError(i + 1, TMath::Sqrt(kNElec_rc[t]));
        }
      }
  ...
  // define canvas
  TCanvas *c = new TCanvas("evnt-mixing_MR_rc", "evnt-mixing_MR_rc", 1080, 1080);
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    ...
    legend->AddEntry(MRgraph[k][1], "C, Rad. Corr.", "p");
    legend->AddEntry(MRgraph[k][2], "Fe, Rad. Corr.", "p");
    legend->AddEntry(MRgraph[k][3], "Pb, Rad. Corr.", "p");
