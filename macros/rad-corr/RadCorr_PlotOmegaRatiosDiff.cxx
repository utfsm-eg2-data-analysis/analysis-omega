...
void RadCorr_PlotOmegaRatiosDiff() {
  ...
  /*** OBTAIN DIFFERENCE OF RATIOS ***/

  Double_t ratioDiffOmega[Nkinvars][Ntargets][Nbins];
  Double_t ratioDiffOmegaError[Nkinvars][Ntargets][Nbins];

  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      for (Int_t i = 0; i < Nbins; i++) {  // i for bins
        ratioDiffOmega[k][t][i] = ratioOmega[k][t][i] - ratioOmega_rc[k][t][i];
        ratioDiffOmegaError[k][t][i] =
            0;  //            TMath::Sqrt(TMath::Power(ratioOmegaError[k][t][i], 2) + TMath::Power(ratioOmegaError_rc[k][t][i], 2));
        ratioDiffOmegaError[k][t][i] = 0;  // ratioDiffOmega[k][t][i] / ratioOmega[k][t][i] *
        // TMath::Sqrt(TMath::Power(ratioDiffOmegaError[k][t][i] / ratioDiffOmega[k][t][i], 2) +
        //          TMath::Power(ratioOmegaError[k][t][i] / ratioOmega[k][t][i], 2));
        ratioDiffOmega[k][t][i] = ratioDiffOmega[k][t][i] / ratioOmega[k][t][i];
      }
    }
  }
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      // nominal
      omegaGraph[k][t] = new TGraphErrors(Nbins, binCenter[k], ratioDiffOmega[k][t], binError[k], ratioDiffOmegaError[k][t]);
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    omegaGraph[k][1]->GetYaxis()->SetRangeUser(-0.01, 0.01);

    omegaGraph[k][1]->GetYaxis()->SetTitle("(R_{#omega} - R_{#omega}^{corr})) / R_{#omega}");
  ...
  TCanvas *c = new TCanvas("rad-corr_omega-ratios-diff", "rad-corr_omega-ratios-diff", 1080, 1080);
