...
void Make_AcceptanceCorrFactors(TString StoreOption = "") {
  // Calculate and draw acceptance of electrons divided by the acceptance of omegas for each target
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t i = 0; i < Nbins; i++) {
      ...
      for (Int_t t = 0; t < Ntargets; t++) {
        acceptanceValue[k][t][i] = AccCorrFactorsHist[k][t]->GetBinContent(i + 1);
        acceptanceError[k][t][i] = AccCorrFactorsHist[k][t]->GetBinError(i + 1);
      }
    }
  }
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    ...
    acceptanceGraph[k][0]->GetYaxis()->SetTitle("A_{e-}/A_{#omega}");
  ...
  TString CanvasName = "acceptance-corr-factors";
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    ...
    // legend
    if (k == 0) {
      TLegend *legend = new TLegend(0.8, 0.7, 0.9, 0.9);  // x1,y1,x2,y2
