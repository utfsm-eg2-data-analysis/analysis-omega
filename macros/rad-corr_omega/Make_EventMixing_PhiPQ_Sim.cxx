...
void Make_EventMixing_PhiPQ_Sim(TString targetOption = "D", Int_t fixParams = 0, TString StoreOption = "") {
  // 1) Normalize event-mixing bkg to match sim on PhiPQ-dependent binning
  // 2) Subtract event-mixing bkg to sim
  ...
  // about kinvar option
  TString kinvarOption = "wPhiPQ";
  TString kinvarTitle = "#phi_{PQ}";
  Double_t EdgesKinvar[Nbins + 1] = {-180, -90, 0, 90, 180};
  ...
  if (fixParams) {
    // read the bkg-subtracted output file when running on "All" sim
    RootInputFile = new TFile(gProDir + "/gfx/rad-corr_omega/evnt-mixing_All_" + kinvarOption + "_sim.root");
    // loop over bins
  ...
  for (Int_t i = 0; i < Nbins; i++) {
    ...
    simMassive[i]->SetLineColor(myBlack);
    simMassive[i]->SetLineWidth(2);
    ...
    simNorm = simMassive[i]->Integral(BinsForNormalization[0], BinsForNormalization[1]) +
              simMassive[i]->Integral(BinsForNormalization[2], BinsForNormalization[3]);
    ...
    bkgMassive[i]->Scale(simNorm / bkgNorm);
  ...
  Double_t MaxRangeSim = 0;
  Double_t MaxRangeSub = 0;
  // loop over bins
  for (Int_t i = 0; i < Nbins; i++) {
    if (simMassive[i]->GetMaximum() > MaxRangeSim) {
      MaxRangeSim = simMassive[i]->GetMaximum();
    }
  ...
  TString CanvasName = "evnt-mixing_" + targetOption + "_" + kinvarOption + "_sim";
  ...
  for (Int_t i = 0; i < Nbins; i++) {
    ...
    subMassive[i]->GetYaxis()->SetTitleOffset(1.05);
    ...
    leg->AddEntry(simMassive[i], "Sim. Rec.", "lp");
    ...
    leg2->AddEntry(subMassive[i], "Sim. Rec. - Bkg", "lp");
