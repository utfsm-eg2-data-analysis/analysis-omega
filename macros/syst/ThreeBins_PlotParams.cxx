...
void ThreeBins_PlotParams() {
  ...
  for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[0][i] = kEdgesQ2_ThreeBins[i];
  for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[1][i] = kEdgesNu_ThreeBins[i];
  for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[2][i] = kEdgesZ_ThreeBins[i];
  for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[3][i] = kEdgesPt2_ThreeBins[i];
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      rootInputFile[k][t] = new TFile("3b/syst-3b_evnt-mixing_" + targetString[t] + "_" + kinvarOption[k] + ".root", "READ");
  ...
  TCanvas *c = new TCanvas("syst-3b_evnt-mixing_plot-params", "syst-3b_evnt-mixing_plot-params", 1080, 1080);
