...
void FiveBins_EventMixing(TString targetOption = "D", TString kinvarOption = "Q2") {
  ...
  if (kinvarOption == "Q2") {
    kinvarTitle = "Q^{2}";
    for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[i] = kEdgesQ2_FiveBins[i];
  } else if (kinvarOption == "Nu") {
    kinvarTitle = "#nu";
    for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[i] = kEdgesNu_FiveBins[i];
  } else if (kinvarOption == "wZ") {
    kinvarTitle = "z_{h}";
    for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[i] = kEdgesZ_FiveBins[i];
  } else if (kinvarOption == "wPt2") {
    kinvarTitle = "p_{T}^{2}";
    for (Int_t i = 0; i < Nbins + 1; i++) EdgesKinvar[i] = kEdgesPt2_FiveBins[i];
  }
  ...
  TFile *RootOutputFile = new TFile("5b/syst-5b_evnt-mixing_" + targetOption + "_" + kinvarOption + ".root", "RECREATE");
  ...
  TCanvas *can1 = new TCanvas("syst-5b_evnt-mixing_" + targetOption + "_" + kinvarOption,
                              "syst-5b_evnt-mixing_" + targetOption + "_" + kinvarOption, 1920, 960);
  ...
  can1->Print("5b/syst-5b_evnt-mixing_" + targetOption + "_" + kinvarOption + ".png");
