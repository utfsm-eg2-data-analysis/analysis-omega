...
void EventMixingSim_AllBins(TString setOption = "old", TString targetOption = "D", TString dirOption = "", Int_t CustomNormalization = 0, Int_t BkgSubtraction = 0, TString particleOption = "omega") {
  // Plot all bins, comparing event-mixing from simulation reconstructed files.
  ...
  for (Int_t j = 0; j < Nkinvars; j++) { // Ny
    for (Int_t i = 0; i < Nbins; i++) { // Nx
      ...
      auxCut += ", " + targetOption + " sim";

      simMassive[i][j]->SetTitle(auxCut);
      simMassive[i][j]->GetXaxis()->SetTitle("Reconstructed Mass #Deltam(#pi^{+}#pi^{-}#pi^{0}) [GeV]");
      simMassive[i][j]->GetYaxis()->SetTitle("Counts");

      bkgTree->Draw(Form("wD>>bkg_%i_%i", i, j) + histProperties, gCutDIS && CutBin && gCutPi0 && gCutRegion && gCutKaons && gCutPhotonsOpAngle, "goff");
