...
void Make_BkgFitting_PhiPQ_Sim(TString targetOption = "C", Int_t fixParams = 0, TString StoreOption = "") {
  ...
  if (fixParams) {
    ...
    RootInputFile = new TFile(gProDir + "/gfx/rad-corr_eta/bkg-fitting_All_" + kinvarOption + "_sim.root");
    // loop over bins
  ...
  for (Int_t i = 0; i < NbinsPhiPQ; i++) {
    ...
    FitResult[i] = model.fitTo(data, Range(plotMin, plotMax), Minos(kTRUE), Strategy(2), Extended(), Save());
  ...
  const Int_t Nx = 3;
  const Int_t Ny = 2;
  TString CanvasName = "bkg-fitting_" + targetOption + "_" + kinvarOption + "_sim";
