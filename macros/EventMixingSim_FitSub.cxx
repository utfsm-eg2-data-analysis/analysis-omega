...
#include "FitGaussian.cxx" // EDIT
#include "GausParams.h"  // struct

#include "DrawHorizontalLine.cxx"

// void EventMixingSim_FitSub(TString targetOption, TString particleOption = "omega") { // this is how it should be in the end
void EventMixingSim_FitSub(TString setOption, TString targetOption, TString dirOption = "", TString particleOption = "omega") {

  // This macro fits the subtracted bkg.

  /*** INPUT ***/
  TString InputFileName = "evnt-mixing-sim-" + particleOption + "_binned_" + setOption + "-" + targetOption;
  if (setOption == "jlab") InputFileName += "-" + dirOption;
  TFile *RootInputFile = new TFile(gProDir + "/macros/out/" + InputFileName + ".root", "READ");

  /*** OUTPUT ***/

  TString OutputFileName = "evnt-mixing-sim-" + particleOption + "_fit-sub_" + setOption + "-" + targetOption;
  ...
  GausParams Parameters[Nbins][Nkinvars]; // EDIT
  ...
  for (Int_t j = 0; j < Nkinvars; j++) {  // Ny
    for (Int_t i = 0; i < Nbins; i++) {   // Nx
      ...
      if (particleOption == "omega") FitGaussian(Form("fit_%d_%d", i, j), CurrentHist, Parameters[i][j]); // EDIT
  ...
  for (Int_t j = 0; j < Nkinvars; j++) {
    for (Int_t i = 0; i < Nbins; i++) {
      ...
      SigmaString = Form("#sigma = %.3f +- %.6f", Parameters[i][j].SigmaValue, Parameters[i][j].SigmaError); // EDIT
      // SigmaString = Form("#sigma = %.3f +- %.6f", Parameters[i][j].GammaValue, Parameters[i][j].GammaError);
