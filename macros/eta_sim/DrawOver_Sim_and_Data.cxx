...
void DrawOver_Sim_and_Data(TString targetOption = "Fe", TString StoreOption = "") {
  ...
  TString kinvarOption[2][3] = {{"Q2", "Nu", "W"}, {"nM", "nZ", "nPt2"}};
  TString histProperties[2][3] = {{"(100, 1., 4.1)", "(100, 2.2, 4.25)", "(100, 2.0, 3.0)"},
                                  {"(54, 0.26, 0.8)", "(100, 0.5, 1.0)", "(100, 0, 1.5)"}};
  TString kinvarAxis[2][3] = {{"Q^{2} [GeV^{2}]", "#nu [GeV]", "W [GeV]"},
                              {"Reconstructed Mass m(#gamma#gamma) [GeV]", "Z_{h}", "p_{T}^{2} [GeV^{2}]"}};
  ...
  for (Int_t i = 0; i < Nx; i++) {
    for (Int_t j = 0; j < Ny; j++) {
      ...
      simChain->Draw(Form(kinvarOption[j][i] + ">>sim_%i_%i" + histProperties[j][i], i, j),
                     gCutDIS && gCutPhotonsOpAngle && gCutRegion_Eta, "goff");
