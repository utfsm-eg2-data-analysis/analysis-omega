...
void Draw_EtaNumbers(TString StoreOption = "") {
  // draw the obtained number of etas after background subtraction via bkg fitting (sim)
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      rootInputFile[k][t] = new TFile(gProDir + "/gfx/eta_sim/bkg-fitting_" + targetString[t] + "_" + kinvarOption[k] + ".root");
      // loop over bins
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    ...
    etaGraph[k][0]->GetYaxis()->SetTitle("N_{#eta}^{rec}");
  ...
  for (Int_t k = 0; k < Nkinvars; k++) {
    for (Int_t t = 0; t < Ntargets; t++) {
      ...
      if (TMath::MaxElement(Nbins, etaGraph[k][t]->GetY()) > MaxRangeSim[k]) {
        MaxRangeSim[k] = TMath::MaxElement(Nbins, etaGraph[k][t]->GetY());
      }
    }
  }
  ...
  TString CanvasName = "eta-numbers_sim";
  TCanvas *c = new TCanvas(CanvasName, CanvasName, 2160, 2160);
  c->Divide(2, 2, 0.001, 0.001);  // nx, ny, margins

  // set y-axis for all plots
  etaGraph[0][0]->GetYaxis()->SetRangeUser(0., 1.2 * MaxRangeSim[0]);
  etaGraph[1][0]->GetYaxis()->SetRangeUser(0., 1.2 * MaxRangeSim[1]);
  etaGraph[2][0]->GetYaxis()->SetRangeUser(0., 1.2 * MaxRangeSim[2]);
  etaGraph[3][0]->GetYaxis()->SetRangeUser(0., 1.2 * MaxRangeSim[3]);
  ...
  legend->AddEntry(etaGraph[0][0], "D (Sim. Rec.)", "pl");
  legend->AddEntry(etaGraph[0][1], "C (Sim. Rec.)", "pl");
  legend->AddEntry(etaGraph[0][2], "Fe (Sim. Rec.)", "pl");
  legend->AddEntry(etaGraph[0][3], "Pb (Sim. Rec.)", "pl");
