...
void MakeMR_EventMixing(TString StoreOption = "") {
  // Calculate and draw MR after background subtraction via event mixing and electron numbers
  ...
  // declare input files
  TFile *HistFile[Ntargets][Nkinvars];
  ...
  for (Int_t t = 0; t < Ntargets; t++) {
    for (Int_t k = 0; k < Nkinvars; k++) {
      ...
      HistFile[t][k] = new TFile(gProDir + "/gfx/omega_evnt-mixing/evnt-mixing_" + targetString[t] + "_" + kinvarOption[k] + ".root");
      ...
      for (Int_t i = 0; i < Nbins; i++) {
        ...
        TF1 *auxFit = (TF1 *)HistFile[t][k]->Get(Form("model_%i", i));
        TH1D *auxHist = (TH1D *)HistFile[t][k]->Get(Form("sub_%i", i));

        Double_t fitMean = auxFit->GetParameter(1);
        Double_t fitSigma = auxFit->GetParameter(2);

        // remove negative entries
        Double_t auxNOmega = 0;
        Double_t auxNOmegaError = 0;
        // loop over hist bins within 3sigma range
        for (Int_t s = TMath::Floor((fitMean - 3 * fitSigma - 0.66) / 0.01); s <= TMath::Ceil((fitMean + 3 * fitSigma - 0.66) / 0.01);
             s++) {
          if (auxHist->GetBinContent(s) > 0) {
            auxNOmega += auxHist->GetBinContent(s);
          }
          if (auxHist->GetBinContent(s) > 0) {
            auxNOmegaError += auxHist->GetBinError(s) * auxHist->GetBinError(s);
          }
        }
        auxNOmegaError = TMath::Sqrt(auxNOmegaError);
