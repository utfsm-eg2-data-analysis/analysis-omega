void CalcKaonContamination_Z() {
  ...
  ntuple_sim->Draw("Zh>>PipFromAny_AM(100, 0, 1.1)", electronCuts && "pid == 211 && mc_pid != -9999", "goff");
  TH1F *PipFromAny_AM = (TH1F *)gROOT->FindObject("PipFromAny_AM");

  ntuple_sim->Draw("Zh>>PipFromKaon(100, 0, 1.1)", electronCuts && "pid == 211 && mc_pid == 321", "goff");
  TH1F *PipFromKaon = (TH1F *)gROOT->FindObject("PipFromKaon");

  TH1F *Ratio = new TH1F("Ratio", "Ratio", 100, 0, 1.1);
  ...
  Ratio->GetXaxis()->SetTitle("Z_{h}");

  Ratio->SetLineColor(kRed);
  Ratio->SetLineWidth(2);
  Ratio->SetFillStyle(0);

  TCanvas *c = new TCanvas("kaonp-contamination_on-Z", "c", 1000, 1000);
  gStyle->SetOptStat(0);
