...
void SimVsData_AllVars(TString targetOption = "C") {
  ...
  TString histProperties[2][3] = {{"(100, 1, 4)", "(100, 2.2, 4.2)", "(100, 2, 3.0)"}, {"(100, 0.5, 0.9)", "(100, 0, 1.5)", "(250, 0., 2.5)"}};
  ...
  for (Int_t i = 0; i < Nx; i++) {    // Nx
    for (Int_t j = 0; j < Ny; j++) {  // Ny
      ...
      simTree->Draw(Form(kinvarOption[j][i] + ">>sim_%i_%i" + histProperties[j][i], i, j), gCutDIS && gCutPi0 && gCutKaons && gCutPhotonsOpAngle,
                    "goff");
  ...
  TCanvas *can1 = new TCanvas("sim-vs-data_all-vars", "sim-vs-data_all-vars", 1500, 1000);
  ...
  for (Int_t j = 0; j < Ny; j++) {
	...
	dataMassive[i][j]->GetYaxis()->SetRangeUser(0., 1.1);
      ...
      legend->AddEntry(simMassive[i][j], "Sim. Rec.", "pl");
